...
for($i=0;$i<$c;$i++)
{
    $numerator=0;
    $denomerator=0;
	$sql = "SELECT hour_count from year".$year_select."_hourwise where date='".$date."' and hour_section='".$section[$i]."'";
	$result = $conn->query($sql);
...
var chart = new CanvasJS.Chart("chartContainer", {
    ...
    theme: "light2",
    title:{
        text: "<?php echo strtoupper($dbname); echo" YEAR-";echo$year_select ?> SECTION WISE ATTENDANCE ON  <?php echo $ex_date[2]."-".$ex_date[1]."-".$ex_date[0];?>"
    },
    legend:{
        cursor: "pointer",
